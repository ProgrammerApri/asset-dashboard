...
const PembelianPC = () => {
  ...
  const editPC = async () => {
    const config = {
      ...endpoints.editPC,
      endpoint: endpoints.editPC.endpoint + currentItem.id,
      data: {
  ...
  const delPC = async (id) => {
    const config = {
      ...endpoints.delPC,
      endpoint: endpoints.delPC.endpoint + currentItem.id,
    };
    console.log(config.data);
    let response = null;
    try {
  ...
  return (
    <>
      ...
      <Dialog
        header={isEdit ? "Edit Pembelian Dengan PC" : "Tambah Pembelian Dengan PC"}
        visible={displayData}
        style={{ width: "50vw" }}
