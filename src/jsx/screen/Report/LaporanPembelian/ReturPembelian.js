...
const ReportReturBeli = () => {
  const [retOrd, setRetOrd] = useState(null);
  ...
  const getRet = async (isUpdate = false) => {
    setLoading(true);
    const config = {
  ...
  const jsonForExcel = (retOrd, excel = false) => {
    let data = [];

    retOrd?.forEach((el) => {
      let dt = new Date(`${el?.ret_date}Z`);
      if (dt >= filtersDate[0] && dt <= filtersDate[1]) {
        let val = [
          {
            ref: `No. Ret : ${el.ret_code}`,
            type: "header",
            value: {
              date: "Retur Date",
              code: "Invoice Code",
        ...
        el.product?.forEach((ek) => {
          val.push({
            type: "item",
            value: {
              date: formatDate(el.ret_date),
              code: el.fk_id.fk_code,
              sup: supp(el.fk_id?.ord_id?.sup_id)?.supplier?.sup_name,
              prod: `${ek.prod_id.name} (${ek.prod_id.code})`,
              ord: ek.retur,
              unit: ek.unit_id.code,
              prc: `Rp. ${formatIdr(ek.price)}`,
              tot: `Rp. ${formatIdr(ek.totl)}`,
            },
          });
          total += ek.totl;
        });
    ...
    return (
      <div className="flex justify-content-between">
        ...
        <Row className="mr-1 mt-2" style={{ height: "3rem" }}>
          <div className="mr-3">
            <ExcelFile
              filename={`purchase_return_report_export_${new Date().getTime()}`}
              element={
                <PrimeSingleButton
                  label="Excel"
            ...
            >
              <ExcelSheet
                dataSet={retOrd ? jsonForExcel(retOrd, true) : null}
  ...
  return (
    <>
      ...
      <Row className="m-0 justify-content-center" ref={printPage}>
        {chunk(jsonForExcel(retOrd) ?? [], chunkSize)?.map((val, idx) => {
          return (
            <Card className="ml-1 mr-1 mt-2">
              <Card.Body className="p-0 m-0">
                <CustomeWrapper
                  tittle={"Purchase Return Report"}
                  subTittle={`Purchase Return Report From ${formatDate(
                    filtersDate[0]
...
export default ReportReturBeli;
